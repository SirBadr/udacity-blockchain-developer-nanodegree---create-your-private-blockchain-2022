    ...
    validateChain(){
        this.app.get("/validatechain", async(req, res) => {
            try{
                ...
                console.log("isValid in API", isValid);
                if(isValid){
                    return res.status(200).send("valid blockchain");
                }else{
                    return res.status(200).send("Invalid Blockchain");
                }
            }catch(e){
                return res.status(500).send("Error validating your private blockchain")
            }
        })
